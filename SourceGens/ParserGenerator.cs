...
public class ParserSourceGenerator : IIncrementalGenerator
{
                                                                       ...
                                                                       ParserClassMetadata classMetadata)
   {
      ...
      var dynamicBlockNodeProps = properties.Where(p => p.IEu5KeyType != null).ToList();
                                                      ...
                                                      ParserClassMetadata classMetadata)
   {
      ...
      foreach (var prop in properties)
      {
         ...
         if (AddDynamicBlockParser(prop, sb, targetTypeName, customParserName))
            continue;
                                             ...
                                             string? customParserName)
   {
      ...
      sb.AppendLine("// ### Dynamic Block Parser ###");
      ...
      sb.AppendLine("    {");
      sb.AppendLine($"        var globals = ((IEu5Object)EmptyRegistry.Empties[typeof({prop.IEu5KeyType.ToDisplayString()})]).GetGlobalItemsNonGeneric();");
      sb.AppendLine($"        var key = node.KeyNode.GetLexeme(source);");
      sb.AppendLine($"        if (!globals.Contains(key))");
      sb.AppendLine("            return false;");
      sb.AppendLine($"        var value = Pdh.ParseDynamicObject<{objStr}>(key, node, source, ctx, {customParserName ?? "MISSING_CUSTOM_PARSER"}.ParseProperties, ref validation);");
                                                     ...
                                                     ImmutableArray<INamedTypeSymbol> allKnownParsers)
   {
      ...
      {
         sb.AppendLine($"        Pdh.ParseProperties(block, target, ctx, source, ref validation, _contentParsers, _blockParsers, _statementParsers, _dynamicBlockParsers, _ignoredBlockTypes, _ignoredContentTypes, allowUnknownNodes);");
      }
