...
public static class Effectparsing
{
    public static List<DocsEffect> ParseEffects(string path)
    {
        var lines = IO.IO.ReadAllLinesUtf8(path);
        if (lines == null || lines.Length == 0)
            return [];
      
        var triggerList = new List<DocsEffect>();
        DocsEffect? currentTrigger = null;
