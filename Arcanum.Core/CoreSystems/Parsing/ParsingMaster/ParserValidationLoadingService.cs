...
public abstract class ParserValidationLoadingService<T> : FileLoadingService where T : IEu5Object<T>, new()
{
   ...
   public override bool LoadSingleFile(FileObj fileObj, FileDescriptor descriptor, object? lockObject = null)
   {
      ...
      LoadSingleFile(rn, ctx, new(fileObj.Path, fileObj.Descriptor), ACTION_STACK, source, ref validation);
   ...
   {
      return UnloadSingleFileContent(new(fileObj.Path, fileObj.Descriptor));
   }

   protected abstract bool UnloadSingleFileContent(Eu5FileObj<T> fileObj);
