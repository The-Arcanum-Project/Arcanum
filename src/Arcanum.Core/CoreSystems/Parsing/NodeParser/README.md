...
Each block and content should provide a line number and char index for each element that carries information so not for
example the opening and closing brackets of a block or the separators.
...
A content is just a collection of key or key-value pairs which are separated by different separators. Otherwise it is a
key collection if none of the separators are present.
