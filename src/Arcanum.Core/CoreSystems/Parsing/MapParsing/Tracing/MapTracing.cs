...
public sealed unsafe class MapTracing : IDisposable
{
    ...
    private PolygonParsing[] VisitNode(Node node, Dictionary<int, List<PolygonParsing>> polygons, Direction[] arriveDirection)
    {
    ...
    private void HandleIsland(Vector2I position, Dictionary<int, List<PolygonParsing>> polygons, Vector2I borderPos,
        int parentColor)
    {
        ...
        var holes = VisitNode(startNode, polygons,startNode == endNode ? [startCache.Dir, startCache.Dir.Invert()] : [startCache.Dir]);
