...
public static class Selection
{
        ...
        bool clearAllFirst = false)
    {
        ...
        if (clearAllFirst)
        {
            var newLocations = locations.ToHashSet();
            foreach (var location in targetSet.Where(location => !newLocations.Contains(location)))
            {
                RemoveCache.Add(location);
            }

            foreach (var location in newLocations.Where(location => !targetSet.Contains(location)))
            {
                AddCache.Add(location);
            }
        }
        else if (additive)
        ...
        var intersection = AddCache.Intersect(RemoveCache).ToHashSet();
        for (var index = AddCache.Count - 1; index >= 0; index--)
        {
            var cache = AddCache[index];
            if (intersection.Contains(cache))
            {
                AddCache.RemoveAt(index);
                targetSet.Add(cache);
            }
        }

        for (var index = RemoveCache.Count - 1; index >= 0; index--)
        {
            var cache = RemoveCache[index];
            if (intersection.Contains(cache))
            {
                RemoveCache.RemoveAt(index);
            }
        }
    ...
    public static void UpdateDragSelection(Vector2 mousePos, bool isDragging, bool isLasso, bool remove = false,
        bool clearAllFirst = false)
    {
    ...
    private static void SetRectanglePreviewLocations(Vector2 first, Vector2 last)
    {
        ...
        else
        {
            ...
            var newArea = new RectangleF(Math.Min(first.X, last.X),
                Math.Min(first.Y, last.Y),
                Math.Abs(first.X - last.X),
                Math.Abs(first.Y - last.Y));

            var rectangleDiff = GeoRect.RectDiff(DragArea, newArea);

            List<Location> added = [];
            List<Location> removed = [];

            if (rectangleDiff.Rect1Added)
            {
                added = MapManager.Lqt.FindLocations(rectangleDiff.Rect1);
            }
            else
            {
                removed = MapManager.Lqt.FindLocations(rectangleDiff.Rect1);
            }

            if (rectangleDiff.Rect2Added)
            {
                var locs = MapManager.Lqt.FindLocations(rectangleDiff.Rect2);
            ...
            // check the removed locations to see if they are still in the new area
            for (var index = removed.Count - 1; index >= 0; index--)
            {
                var remove = removed[index];
                if (remove.Polygons.Any(p => p.Intersects(newArea)))
                {
                    removed.RemoveAt(index);
                }
            }

            Debug.WriteLine($"Added {added.Count} items");
            Debug.WriteLine($"Removed {removed.Count} items");

            Modify(SelectionTarget.SelectionPreview, SelectionMethod.Rectangle, removed, false, false);
            Modify(SelectionTarget.SelectionPreview, SelectionMethod.Rectangle, added, true, invert: false);
    ...
    public static List<Location> GetLocations(Vector2 vec2, float radius)
    {
        return MapManager.Lqt.FindLocations(vec2, radius);
    }
}