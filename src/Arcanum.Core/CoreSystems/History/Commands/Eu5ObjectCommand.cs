...
public abstract class Eu5ObjectCommand : ICommand
{
    ...
    public bool DisallowValueReplace(IEu5Object[] target, Enum attribute)
    {
        Debug.Assert(target.Length == 0 || target.All(x => x.GetType() == target[0].GetType()));
        return target.Length == 0 || target[0].GetType() != Type || !attribute.Equals(Attribute);
    }
