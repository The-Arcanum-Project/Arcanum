...
public class APIWrapperJsonProcessor : IJsonProcessor
{
   ...
   public T? Deserialize<T>(string json, JsonDeserializationRules? rules = null) => JsonProcessor.Deserialize<T>(json, rules);

   public bool TryDeserialize<T>(string json, out T? value, JsonDeserializationRules? rules = null) => JsonProcessor.TryDeserialize(json, out value, rules);

   public Task<string> SerializeAsync<T>(T value,
                                         JsonSerializationRules? rules = null,
                                         CancellationToken cancellationToken = default) => JsonProcessor.SerializeAsync(value, rules, cancellationToken);

   public Task<T?> DeserializeAsync<T>(string json,
                                       JsonDeserializationRules? rules = null,
                                       CancellationToken cancellationToken = default) => JsonProcessor.DeserializeAsync<T>(json, rules, cancellationToken);
