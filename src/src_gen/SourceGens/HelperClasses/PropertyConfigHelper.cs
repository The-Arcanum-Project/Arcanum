...
public static class PropertyConfigHelper
{
                                                               ...
                                                               SourceProductionContext context)
   {
      ...
      else
      {
         ...
         var minValueArg = (minValueIndex != -1 && minValueIndex < pcad.ConstructorArguments.Length)
                              ? pcad.ConstructorArguments[minValueIndex]
                              : default;

         var maxValueArg = (maxValueIndex != -1 && maxValueIndex < pcad.ConstructorArguments.Length)
                              ? pcad.ConstructorArguments[maxValueIndex]
                              : default;

         pcd = new()
         {
            PropertyType = propSymbol.Type,
            IsReadonly = AttributeHelper.SimpleGetAttrArgValue<bool>(pcad, 0, "isReadonly"),
            IsInlined = AttributeHelper.SimpleGetAttrArgValue<bool>(pcad, 1, "isInlined"),
            AllowEmpty = AttributeHelper.SimpleGetAttrArgValue<bool>(pcad, 2, "allowEmpty"),
            DisableMapInferButtons = AttributeHelper.SimpleGetAttrArgValue<bool>(pcad, 3, "disableMapInferButtons"),
            IsRequired = AttributeHelper.SimpleGetAttrArgValue<bool>(pcad, 4, "isRequired"),

            // Use the constructor arguments we just found.
            MinValue = FormatTypedConstant(minValueArg),
            MaxValue = FormatTypedConstant(maxValueArg),
            PropertyName = propSymbol.Name,
         };
      }
