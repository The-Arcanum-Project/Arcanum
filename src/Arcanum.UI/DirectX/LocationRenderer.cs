...
public class LocationRenderer(VertexPositionId2D[] vertices, Color4[] initColors, float imageAspectRatio) : ID3DRenderer
{
    ...
    public void Initialize(IntPtr hwnd, int width, int height)
    {
      ...
      D3D11.D3D11CreateDeviceAndSwapChain(null,
        ...
        _outlineVertexBuffer = _device.CreateBuffer(new BufferDescription(
            (uint)(MAX_OUTLINE_VERTICES * VertexPosition2D.SizeInBytes), BindFlags.VertexBuffer, ResourceUsage.Dynamic,
            CpuAccessFlags.Write));
    ...
    {
        if (_context == null || _outlineVertexBuffer == null) return false;
    ...
    {
        if (_context == null || _outlineVertexBuffer == null) return;
        ...
        {
            Unsafe.CopyBlockUnaligned(mapped.DataPointer.ToPointer(), pVertices, (uint)(_outlineVertexCount * sizeof(Vector2)));
        }
        _context.Unmap(_outlineVertexBuffer);
    }
