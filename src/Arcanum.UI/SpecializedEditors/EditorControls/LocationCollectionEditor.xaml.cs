...
public partial class LocationCollectionEditor
{
   ...
   private void OnCreateObjectButton_Click(object sender, RoutedEventArgs e)
   {
      var newObj = Eu5ObjectCreator.ShowPopUp(_currentParentType, PostCreationAction);

      switch (newObj)
      {
         case Province p:
            SetLocationCollection(p, p.Locations, _editor);
            break;
         case Area a:
            SetLocationCollection(a, a.Provinces, _editor);
            break;
         case Region r:
            SetLocationCollection(r, r.Areas, _editor);
            break;
         case SuperRegion s:
   ...
   private static void PostCreationAction(IEu5Object obj)
   {
      ...
      {
         case Province p:
            Nx.AddRangeToCollection(p, Province.Field.Locations, ((IMapInferable)Location.Empty).GetInferredFromSelection());
            break;
         case Area a:
            Nx.AddRangeToCollection(a, Area.Field.Provinces, ((IMapInferable)Province.Empty).GetInferredList(Selection.GetSelectedLocations));
            break;
         case Region r:
            Nx.AddRangeToCollection(r, Region.Field.Areas, ((IMapInferable)Area.Empty).GetInferredList(Selection.GetSelectedLocations));
            break;
         case SuperRegion s:
            Nx.AddRangeToCollection(s, SuperRegion.Field.Regions, ((IMapInferable)Region.Empty).GetInferredList(Selection.GetSelectedLocations));
            break;
         case Continent c:
            Nx.AddRangeToCollection(c, Continent.Field.SuperRegions, ((IMapInferable)SuperRegion.Empty).GetInferredList(Selection.GetSelectedLocations));
            break;
      }
   }
}