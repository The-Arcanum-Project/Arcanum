...
public class MapCoordinateConverter(MapControl mapControl, (int width, int height) imageSize)
{
   ...
   public void InvalidateCache(Vector2 pixelPoint)
   {
      ...
      CurrentPosition = new(Map: mapPoint,
                            Norm: normPoint,
                            Ndc: ndcPoint,
                            Pixel: pixelPoint);
   ...
   {
      return new(pixelPoint.X / (float)mapControl.HwndHostContainer.ActualWidth * 2.0f - 1.0f,
   ...
   {
      return new((ndcPoint.X + 1.0f) / 2.0f * (float)mapControl.HwndHostContainer.ActualWidth,
   ...
   {
      return new(normPoint.X * imageSize.width,
                 normPoint.Y * imageSize.height);
   }

   public Vector2 MapToNorm(Vector2 mapPoint)
   {
      return new(mapPoint.X / imageSize.width,
