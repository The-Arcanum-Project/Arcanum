...
public class MapNavigationStrategy(MapInteractionManager mapInteractionManager) : IMapInteractionStrategy
{
   ...
   public void OnMouseMove(MapControl map, MouseEventArgs e)
   {
      ...
      var panX = map.LocationRenderer.Pan.X -
                 (float)(deltaX / (map.HwndHostContainer.ActualWidth * map.LocationRenderer.Zoom)) *
                 map.Coords.ImageAspectRatio *
                 aspectRatio;
      var panY = map.LocationRenderer.Pan.Y -
                 (float)(deltaY / (map.HwndHostContainer.ActualHeight * map.LocationRenderer.Zoom));
   ...
   public void OnMouseWheel(MapControl map, MouseWheelEventArgs e)
   {
      ...
      var delta = new Vector2(mapPos.X - map.LocationRenderer.Pan.X, mapPos.Y - map.LocationRenderer.Pan.Y);
      delta /= zoomFactor;
