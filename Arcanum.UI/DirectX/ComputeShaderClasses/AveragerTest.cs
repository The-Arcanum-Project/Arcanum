...
public static class AveragerTest
{
   public static void RunTest()
   {
      ...
      // 2. Calculate dispatch dimensions.
                                                   ...
                                                   Image<Rgba32> image)
   {
      ...
      using var inputLayout = renderer.Device.CreateInputLayout(inputElements, vsByteCode.Span);
      ...
      renderer.Render(renderTarget: renderTargetView,
                      vertexShader: vertexShader,
                      pixelShader: pixelShader,
                      inputLayout: inputLayout,
                      vertexBuffers: [vertexBuffer, instanceBuffer],
                      strides: [(uint)Marshal.SizeOf<Vertex>(), (uint)Marshal.SizeOf<InstanceData>()],
                      indexBuffer: indexBuffer,
                      constantBuffer: constantBuffer,
                      topology: PrimitiveTopology.TriangleList,
                      indexCount: (uint)quadIndices.Length,
                      instanceCount: (uint)locations.Length);
